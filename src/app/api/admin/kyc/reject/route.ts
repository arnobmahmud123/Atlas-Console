...
export async function POST(request: Request) {
  ...
  if (contentType.includes('application/json')) {
    ...
    raw = { kycId: form.get('kycId'), id: form.get('id'), reviewNotes: form.get('reviewNotes') };
  }
  ...
  const kyc = await prisma.kyc.update({
    where: { id: kycId },
    data: { status: 'REJECTED', reviewed_by: session.user.id, review_notes: parsed.data.reviewNotes }
  });

  const user = await prisma.user.findUnique({ where: { id: kyc.user_id }, select: { email: true } });
  let emailWarning: string | null = null;
  if (user?.email) {
    const tmpl = emailKycStatus('REJECTED');
    try {
  ...
  return NextResponse.json({
    ok: true,
    message: emailWarning ? 'KYC rejected (email not sent)' : 'KYC rejected',
    kyc,

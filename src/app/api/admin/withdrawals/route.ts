...
export async function GET(request: Request) {
  ...
  const status =
    parsed.data.status && (Object.values(WithdrawalStatus) as string[]).includes(parsed.data.status)
      ? (parsed.data.status as WithdrawalStatus)
      : undefined;
  ...
  const withdrawals = await prisma.withdrawal.findMany({
    where: {
      ...
      ...(parsed.data.email ? { User_Withdrawal_user_idToUser: { email: { contains: parsed.data.email, mode: 'insensitive' } } } : {})
    },

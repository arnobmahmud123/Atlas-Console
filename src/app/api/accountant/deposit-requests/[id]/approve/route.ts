...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const req = await prisma.depositRequest.findUnique({ where: { id } });
    if (!req) return NextResponse.json({ ok: false, message: 'Deposit request not found' }, { status: 404 });
    ...
    await sendNotification(
      updated.user_id,
      'INFO',
      'Deposit Under Final Review',
      `Your deposit request ${updated.id} was approved by accountant and is now waiting for admin final approval.`,

...
export async function GET(request: Request) {
  ...
  const status =
    (Object.values(DepositRequestStatus) as string[]).includes(statusRaw)
      ? (statusRaw as DepositRequestStatus)
      : DepositRequestStatus.PENDING_ACCOUNTANT;
  ...
  if (format === 'csv') {
    ...
    return new Response([header.join(','), ...lines].join('\n'), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename=\"accountant-deposits-${status.toLowerCase()}-p${page}.csv\"`
      }
    });
  }
